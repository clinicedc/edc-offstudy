...
class Offstudy:
        ...
        **kwargs,
        ...
        appointment_model_cls = django_apps.get_model(
            app_config.get_configuration(
                related_visit_model=self.visit_model_cls._meta.label_lower
            ).model
        )
    ...
    def registered_or_raise(self, **kwargs):
        ...
        try:
            ...
            if not obj.dob:
                raise OffstudyError("Invalid date of birth. Got None", code=INVALID_DOB)
            else:
                if obj.dob > timezone.localdate(self.offstudy_datetime):
                    formatted_date = timezone.localtime(
                        self.offstudy_datetime
                    ).strftime(EDC_DATETIME_FORMAT)
                    raise OffstudyError(
    ...
    def offstudy_datetime_or_raise(self, **kwargs):
        ...
        consent = (
            self.consent_model_cls.objects.filter(
                subject_identifier=self.subject_identifier,
        ...
        last_visit = (
            self.visit_model_cls.objects.filter(
                subject_identifier=self.subject_identifier
            )
        ...
        if (
            last_visit
            and (last_visit.report_datetime - self.offstudy_datetime).days > 0
        ):
            formatted_visitdate = timezone.localtime(
                last_visit.report_datetime
            ).strftime(EDC_DATETIME_FORMAT)
            formatted_offstudy = timezone.localtime(self.offstudy_datetime).strftime(
                EDC_DATETIME_FORMAT
            )
            raise OffstudyError(
