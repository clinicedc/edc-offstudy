...
class TestOffstudy(TestCase):
    ...
    def test_offstudy_cls_subject_not_registered(self):
        ...
        self.assertEqual(cm.exception.code, SUBJECT_NOT_REGISTERED)
    ...
    def test_offstudy_cls_subject_registered_but_no_dob(self):
        ...
        self.assertEqual(cm.exception.code, INVALID_DOB)

    def test_offstudy_cls_subject_registered_but_dob_after_offstudy_datetime(self):
        subject_identifier = "12345"
        RegisteredSubject.objects.create(
            subject_identifier=subject_identifier, dob=self.consent_datetime
        )
        with self.assertRaises(OffstudyError) as cm:
            Offstudy(
                subject_identifier=subject_identifier,
        ...
        self.assertEqual(cm.exception.code, OFFSTUDY_DATETIME_BEFORE_DOB)
    ...
    def test_crf_model_mixin(self):
        ...
        # show CRF saves OK
        crf_one = CrfOne(
            report_datetime=appointment.appt_datetime, subject_visit=subject_visit
        )
        crf_one.save()
        ...
        self.assertRaises(SubjectOffstudyError, crf_one.save)
    ...
    def test_non_crf_model_mixin(self):
        ...
        self.assertRaises(NotImplementedError, getattr, non_crf_one, "schedule")
    ...
    def test_modelform_mixin_not_ok(self):
        ...
        form = SubjectOffstudyForm(data=data)
        form.is_valid()
        self.assertIn("Unknown subject. Got 12345.", form.errors.get("__all__"))
    ...
    def test_crf_model_mixin_for_visit_schedule_2(self):
        ...
        appointment = appointments[0]
        self.assertEqual(
            [a.visit_code for a in appointments], ["1000", "2000", "3000", "4000"]
        )
        ...
        self.assertEqual(appointments, ["1000", "2000", "5000", "6000", "7000", "8000"])

        # show adds the visits for visit_schedule2
        appointments = [
            appt.visit_schedule_name
            for appt in Appointment.objects.filter(
                subject_identifier=self.subject_identifier
            ).order_by("timepoint", "visit_code_sequence")
        ]
