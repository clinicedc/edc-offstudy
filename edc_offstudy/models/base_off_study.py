...
from edc_registration.models import BaseRegisteredSubjectModel
...
class BaseOffStudy(BaseRegisteredSubjectModel):
    ...
    def check_off_study_date(self, subject_identifier, off_study_date, exception_cls=None):
        ...
        app = apps.get_app(self._meta.app_label)
            ...
            if issubclass(model, BaseVisitTracking):
                ...
                if max_report_datetime:
                    ...
                    if off_study_date < report_date:
                        raise exception_cls(
                            'Data exists for this subject with a report datetime AFTER the off study '
                            'date of {0}. See {1} with report_datetime {2}.'.format(
                                off_study_date, model._meta.object_name, max_report_datetime))
    ...
    def post_save_clear_future_appointments(self):
        """Deletes appointments created after the off-study datetime if the
        appointment has no visit report."""
        Appointment = apps.get_model('appointment', 'appointment')
        visit_model_cls = self.get_visit_model_cls()
        if not visit_model_cls:
            raise ImproperlyConfigured(
                'Model {0} cannot determine the visit model class for the app'.format(self._meta.object_name))
        if visit_model_cls:
    ...
    def __str__(self):
        return "{0} {1} ({2})".format(
            ...
            self.registered_subject.first_name.field_cryptor.mask(self.registered_subject.first_name)
        )
